...
class GraphNN(nn.Module):
  ...
  def __init__(self, 
    ...
    #2 GAT layer
    if gnn_type == "GAT": 
      self.conv1 = GATv2Conv(feature_size, hidden_conv[0], add_self_loops = False)
    elif gnn_type == "SAGEConv": 
      self.conv1 = SAGEConv(feature_size, hidden_conv[0])
    elif gnn_type == "GCNConv": 
      self.conv1 = GCNConv(feature_size, hidden_conv[0], add_self_loops = False) 
    self.conv1bn = gnn.BatchNorm(hidden_conv[0])
    
    if gnn_type == "GAT": 
      self.conv2 = GATv2Conv(hidden_conv[0], hidden_conv[1], add_self_loops = False) 
    elif gnn_type == "SAGEConv": 
    ...
    self.norm1 = nn.LayerNorm(hidden_final[0])
    self.linear2 = nn.Linear(hidden_final[0], hidden_final[1])
    self.norm2 = nn.LayerNorm(hidden_final[1])
    self.linear3 = nn.Linear(hidden_final[1], 1)
  
  def forward(self, node_feature, edge_index, global_x):
        
    x, edge_index,global_x = node_feature.to(self.device), edge_index.to(torch.long).to(self.device),global_x.to(self.device)
    #MLP for Global Features
    if self.global_size != 0:
      global_x = F.elu(self.linear_global1(global_x))
      global_x = F.elu(self.linear_global2(global_x))
      if global_x.dim() == 1:
          global_x = global_x.unsqueeze(0)
      global_x = global_x.expand(x.size(0), -1)
    ...
    if self.global_size != 0:
      x = F.elu(self.linear1(torch.cat([x, global_x], dim=-1)))
    else:
